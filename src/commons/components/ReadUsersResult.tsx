...
interface ReadUsersResultProps {
    result: any;
}

const ReadUsersResult = ({
    result
}: ReadUsersResultProps) => {
    if (!result) {
        return null;
    }
    return (
        <div className={`mt-6 p-4 rounded-md ${result.found_users && result.found_users.length > 0 ? 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800' : 'bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800'}`}>
...
export default ReadUsersResult;
